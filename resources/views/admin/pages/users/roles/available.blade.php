...
@section('title', "Cargos Disponiveis para o Cargo {$user->name}")
...
@section('content_header')
    ...
    <h1>Cargos Disponiveis para o Cargo {{ $user->name }}</h1>
    ...
    <div class="card">
        <div class="card-header">
            <form action="{{ route('users.roles.available', $user->id) }}" method="POST" class="form form-inline">
                @csrf
                <input type="text" name="filter" placeholder="Nome/Descrição" class="form-control" value="{{ $filters['filter'] ?? '' }}">
